...
func (r *ReviewRatingRepositoryImpl) Update(reviewRating *entity.ReviewRating) error {
	// Update query
	updateQuery := `UPDATE review_rating 
	                SET user_id = $1, product_id = $2, rating = $3, comment = $4, updated_at = $5 
	                WHERE id = $6`
	...
	selectQuery := `SELECT id, user_id, product_id, rating, comment, created_at, updated_at 
	                FROM review_rating WHERE id = $1`
	row := r.db.QueryRow(selectQuery, reviewRating.ID)

	// Map the values to the reviewRating object
	err = row.Scan(
		&reviewRating.ID,
		&reviewRating.UserID,
		&reviewRating.ProductID,
		...
		&reviewRating.UpdatedAt,
	)
	if err != nil {
		log.Printf("Error fetching updated review rating: %v", err)
		return err
	}
